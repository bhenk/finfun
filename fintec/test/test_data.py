...
class TestData(unittest.TestCase):
    ...
    #     self.assertEqual(ft._data_path('foo.txt'), 'data/foo.txt')
    #     os.environ[ft.U_FIN_DATA_BASE] = 'test-data'
    #     self.assertEqual(ft._data_path('foo.txt'), 'test-data/foo.txt')
    #     del os.environ[ft.U_FIN_DATA_BASE]
    #     self.assertEqual(ft._data_path('foo.txt'), 'data/foo.txt')
    #
    # def test_read_data(self):
    #     df = ft._read_data('fondsen.xlsx')
    #     self.assertListEqual(list(df.columns), ['msuaf', 'nngf', 'nnitf', 'rgfte', 'smwtf'])
    #
    #     df = ft._read_data('rates.csv')
    #     self.assertListEqual(list(df.columns), ['msuaf', 'nngf', 'nnitf', 'rgfte', 'smwtf'])
    #
    # def test_read_data_with_str_index(self):
    #     df = ft._read_data('fondsen.xlsx', index_col='msuaf')
    #     self.assertIsInstance(df.index, pd.Float64Index)
    #
    #     df = ft._read_data('rates.csv', index_col='msuaf')
    #     self.assertIsInstance(df.index, pd.Float64Index)
    #
    # def test_read_data_with_seq_index(self):
    #     df = ft._read_data('fondsen.xlsx', index_col=[0, 3])
    #     self.assertIsInstance(df.index, pd.MultiIndex)
    #
    #     df = ft._read_data('rates.csv', index_col=[0, 3])
    #     self.assertIsInstance(df.index, pd.MultiIndex)
    #
    # def test_read_data_with_no_index(self):
    #     df = ft._read_data('fondsen.xlsx', index_col=None)
    #     self.assertIsInstance(df.index, pd.RangeIndex)
    #
    #     df = ft._read_data('rates.csv', index_col=False)
    #     self.assertIsInstance(df.index, pd.RangeIndex)
